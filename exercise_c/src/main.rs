...
use exercise_c::{print_difference, print_array, ding, on_off, print_distance};

fn main() {
    let coords: (f32, f32) = (6.3, 15.0);
    // 1. Pass parts of `coords` to the `print_difference` function.
    // This should show the difference
    // between the two numbers in coords when you do `cargo run`.
    // Use tuple indexing.
    //
    // The `print_difference` function is defined below the `main` function.
    // It may help if you look
    // at how it is defined.
    //
    print_difference(coords.0, coords.1);   // Uncomment and finish this line


    // 2. We want to use the `print_array` function to print coords...
    // but coords isn't an array!
    ...
    // 3. Make the `ding` function happy by passing it the value 13
    // out of the `series` array.
    // Use array indexing.  Done correctly, `cargo run` will produce
    ...
    // 4. Pass the `on_off` function the value `true` from the variable `mess`.
    //  Done correctly,
    // `cargo run` will produce the additional output "Lights are on!"
    ...
    //
    // `cargo run` should produce the same output, only now the code is
    // more organized. 🎉
