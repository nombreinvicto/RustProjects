...
// - `Miss`
//
// You will need to complete 1b as well before you will be able
...
fn main() {
    // Simulate shooting a bunch of arrows and gathering their
    // coordinates on the target.

    // array_coords is a vector of Coord structs
    let arrow_coords: Vec<Coord> = get_arrow_coords(5);

    // shots is a vector of Shot enums
    let mut shots: Vec<Shot> = Vec::new();

    // 2. For each coord in arrow_coords:
    //
    //   A. Call `coord.print_description()`
    //   B. Append the correct variant of `Shot` to the `shots`
    // vector depending on the value of
    ...
    for coord in arrow_coords {
        ...
        } else if distance_from_center <= 5.0 && distance_from_center > 1.0 {
...
fn get_arrow_coords(num: u32) -> Vec<Coord> {
    ...
    // coords is a vector of the struct Coord
    // struct Coord has xand y coord f64
    // num is how many coord structs u need
